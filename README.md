...
Если задан `API_AUTH_KEY`, консоль один раз спросит ключ и обменяет его на сессионную куку `cp_session` (`HttpOnly`, `SameSite=Strict`, полгода). Дальше браузер подставляет её сам — вводить ключ повторно не нужно, и JavaScript его не видит. Скрипты и CLI продолжают работать через заголовок `X-API-Key`.

Выйти: `DELETE /auth/session`.
...
- `POST /auth/session` — вход по ключу, `DELETE /auth/session` — выход.
